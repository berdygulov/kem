...
$opt_name = 'kem_loc';  // YOU MUST CHANGE THIS.  DO NOT USE 'kem-_loc' IN YOUR PROJECT!!!
...
$args = array(
	...
	'menu_title'                => esc_html__( 'Sample Options', 'kem-local' ),

	// The text to appear on the page title.
	'page_title'                => esc_html__( 'Sample Options', 'kem-local' ),
...
$args['admin_bar_links'][] = array(
	'id'    => 'redux-docs',
	'href'  => '//docs.redux.io/',
	'title' => __( 'Documentation', 'kem-local' ),
);

$args['admin_bar_links'][] = array(
	'id'    => 'redux-support',
	'href'  => '//github.com/ReduxFramework/redux-framework/issues',
	'title' => __( 'Support', 'kem-local' ),
);

$args['admin_bar_links'][] = array(
	'id'    => 'redux-extensions',
	'href'  => 'redux.io/extensions',
	'title' => __( 'Extensions', 'kem-local' ),
);
...
if ( ! isset( $args['global_variable'] ) || false !== $args['global_variable'] ) {
	...
	$args['intro_text'] = '<p>' . sprintf( esc_html__( 'Did you know that Redux sets a global variable for you? To access any of your saved options from within your code you can use your global variable: $%1$s', 'kem-local' ), '<strong>' . $v . '</strong>' ) . '<p>';
} else {
	$args['intro_text'] = '<p>' . esc_html__( 'This text is displayed above the options panel. It isn\'t required, but more info is always better! The intro_text field accepts all HTML.', 'kem-local' ) . '</p>';
}

// Add content after the form.
$args['footer_text'] = '<p>' . esc_html__( 'This text is displayed below the options panel. It isn\'t required, but more info is always better! The footer_text field accepts all HTML.', 'kem-local' ) . '</p>';
...
$help_tabs = array(
	array(
		'id'      => 'redux-help-tab-1',
		'title'   => esc_html__( 'Theme Information 1', 'kem-local' ),
		'content' => '<p>' . esc_html__( 'This is the tab content, HTML is allowed.', 'kem-local' ) . '</p>',
	),
	array(
		'id'      => 'redux-help-tab-2',
		'title'   => esc_html__( 'Theme Information 2', 'kem-local' ),
		'content' => '<p>' . esc_html__( 'This is the tab content, HTML is allowed.', 'kem-local' ) . '</p>',
	),
);
...
$content = '<p>' . esc_html__( 'This is the sidebar content, HTML is allowed.', 'kem-local' ) . '</p>';
...
Redux::set_section(
	$opt_name,
	array(
		'title'            => esc_html__( 'Бренды', 'kem-local' ),
		'id'               => 'brands',
		'desc'             => esc_html__( 'Это базовые поля', 'kem-local' ),
...
Redux::set_section(
	$opt_name,
	array(
		'title'      => esc_html__( 'Логотипы', 'kem-local' ),
		'id'         => 'brands-logos',
		'desc'       => esc_html__( 'Загрузить новое Лого', 'kem-local' ),
		'subsection' => true,
		'fields'     => array(
			array(
				'id'           => 'header-logo',
				'type'         => 'media',
				'url'          => true,
				'title'        => esc_html__( 'Логотип шапки', 'kem-local' ),
				'desc'         => esc_html__( 'Загрузите новое лого для шапки использую это поле', 'kem-local' ),
				'subtitle'     => esc_html__( 'Рекомендуемый рахмер 62x40', 'kem-local' ),
				'preview_size' => 'full',
			),
			array(
				'id'       => 'footer-logo',
				'type'     => 'media',
				'title'    => esc_html__( 'Логотип для футера', 'kem-local' ),
				'desc'     => esc_html__( 'Загрузите новое лого для футера использую это поле', 'kem-local' ),
				'subtitle' => esc_html__( 'Рекомендуемый рахмер 55x35', 'kem-local' ),
				'url'      => true,
				'preview_size' => 'full',
			),
			array(
				'id'       => 'powered-by-logo',
				'type'     => 'media',
				'title'    => esc_html__( 'Логотип разработчика', 'kem-local' ),
				'desc'     => esc_html__( 'Это поле содержит логотип разработчика', 'kem-local' ),
				'subtitle' => esc_html__( 'Рекомендуемый рахмер 110x30', 'kem-local' ),
				'url'      => true,
				'preview_size' => 'full',
			),
			// array(
			...
			// 	'type'     => 'media',
			// 	'preview'  => false,
			// 	'title'    => esc_html__( 'Media No Preview', 'kem-local' ),
			// 	'desc'     => esc_html__( 'This represents the minimalistic view. It does not have the preview box or the display URL in an input box. ', 'kem-local' ),
			// 	'subtitle' => esc_html__( 'Upload any media using the WordPress native uploader', 'kem-local' ),
			// 	'hint'     => array(
			// 		'title'   => esc_html__( 'Test Hint', 'kem-local' ),
			...
			// 	'type'       => 'media',
			// 	'title'      => esc_html__( 'Upload Anything - Disabled Mode', 'kem-local' ),
			// 	'full_width' => true,

			// 	// Can be set to false to allow any media type, or can also be set to any mime type.
			// 	'mode'       => false,

			// 	'desc'       => esc_html__( 'Basic media uploader with disabled URL input field.', 'kem-local' ),
			// 	'subtitle'   => esc_html__( 'Upload any media using the WordPress native uploader', 'kem-local' ),
			// ),
		),
	)
...
Redux::set_section(
	$opt_name,
	array(
		'title'            => esc_html__( 'Опций для Футера', 'kem-local' ),
		'id'               => 'footer-options',
		'desc'             => esc_html__( 'Основные области для футера', 'kem-local' ),
		'customizer_width' => '400px',
...
Redux::set_section(
	$opt_name,
	array(
		'title'      => esc_html__( 'Текст футера', 'kem-local' ),
		'id'         => 'footer-text',
		'desc'       => esc_html__( 'Измените текстовые области для футера', 'kem-local' ),
		'subsection' => true,
		'fields'     => array(
			array(
				'id'           => 'footer-heading',
				'type'         => 'text',
				'title'        => esc_html__( 'Заголовок для футера', 'kem-local' ),
				'desc'         => esc_html__( 'Измените заголов, используя это поле', 'kem-local' ),
				'subtitle'     => esc_html__( 'Заголовок для футера должен не превышать 40 символов с пробелами', 'kem-local' ),
				'placeholder'     => esc_html__( 'Поле для ввода ', 'kem-local' ),
			),
			array(
				'id'           => 'footer-copyright',
				'type'         => 'text',
				'title'        => esc_html__( 'Авторское право для футера', 'kem-local' ),
				'desc'         => esc_html__( 'Измените текст, используя это поле', 'kem-local' ),
				'subtitle'     => esc_html__( 'Текст для авторских прав', 'kem-local' ),
				'placeholder'     => esc_html__( 'Поле для ввода ', 'kem-local' ),
			),
			array(
				'id'           => 'powered-by-link',
				'type'         => 'text',
				'title'        => esc_html__( 'Ссылка на разработчика', 'kem-local' ),
				'desc'         => esc_html__( 'Ссылка при нажатий на лого разработчика', 'kem-local' ),
				'subtitle'     => esc_html__( 'В этом поле присутствует валидация ссылки', 'kem-local' ),
				'placeholder'     => esc_html__( 'Поле для ввода ', 'kem-local' ),
				'validate' => 'url',
			),
			// array(
			// 	'id'       => 'footer-logo',
			// 	'type'     => 'media',
			// 	'title'    => esc_html__( 'Footer logo', 'kem-local' ),
			// 	'desc'     => esc_html__( 'Upload new logo for footer using this field', 'kem-local' ),
			// 	'subtitle' => esc_html__( 'Recomended size 62x40', 'kem-local' ),
			// 	'url'      => true,
			// 	'preview_size' => 'full',
			// ),
		),
...
Redux::set_section(
	$opt_name,
	array(
		'title'      => esc_html__( 'Социальные сети для футера', 'kem-local' ),
		'id'         => 'footer-solials',
		'desc'       => esc_html__( 'Измените ссылки для социальные сетей для футера', 'kem-local' ),
		'subsection' => true,
		'fields'     => array(
			array(
				'id'           => 'footer-solials-whatsapp',
				'type'         => 'text',
				'title'        => esc_html__( 'Ссылка для ватсап', 'kem-local' ),
				'desc'         => esc_html__( 'Измените ссылку на ватсап, используя это поле', 'kem-local' ),
				'subtitle'     => esc_html__( 'Ссылка', 'kem-local' ),
				'placeholder'     => esc_html__( 'Поле для ввода ', 'kem-local' ),
				'validate' => 'url',
			),
			array(
				'id'           => 'footer-solials-instagram',
				'type'         => 'text',
				'title'        => esc_html__( 'Ссылка для инстаграм', 'kem-local' ),
				'desc'         => esc_html__( 'Измените ссылку на инстаграм, используя это поле', 'kem-local' ),
				'subtitle'     => esc_html__( 'Ссылка', 'kem-local' ),
				'placeholder'     => esc_html__( 'Поле для ввода ', 'kem-local' ),
				'validate' => 'url',
			),
			array(
				'id'           => 'footer-solials-telegram',
				'type'         => 'text',
				'title'        => esc_html__( 'Ссылка на телеграм', 'kem-local' ),
				'desc'         => esc_html__( 'Измените ссылку на телеграм, используя это поле', 'kem-local' ),
				'subtitle'     => esc_html__( 'Ссылка', 'kem-local' ),
				'placeholder'     => esc_html__( 'Поле для ввода ', 'kem-local' ),
				'validate' => 'url',
			),
			array(
				'id'           => 'footer-solials-twitter',
				'type'         => 'text',
				'title'        => esc_html__( 'Ссылка на твитер', 'kem-local' ),
				'desc'         => esc_html__( 'Измените ссылку на твитер, используя это поле', 'kem-local' ),
				'subtitle'     => esc_html__( 'Ссылка', 'kem-local' ),
				'placeholder'     => esc_html__( 'Поле для ввода ', 'kem-local' ),
				'validate' => 'url',
			),
...
Redux::set_section(
	$opt_name,
	array(
		'title'      => esc_html__( 'Контакты для футера', 'kem-local' ),
		'id'         => 'footer-contacts',
		'desc'       => esc_html__( 'Измените контактную информацию в футере', 'kem-local' ),
		'subsection' => true,
		'fields'     => array(
			array(
				'id'           => 'footer-contacts-adress',
				'type'         => 'text',
				'title'        => esc_html__( 'Адрес компаний', 'kem-local' ),
				'desc'         => esc_html__( 'Измените адрес компаний, используя это поле', 'kem-local' ),
				'subtitle'     => esc_html__( 'Адрес', 'kem-local' ),
				'placeholder'  => esc_html__( 'Поле для ввода ', 'kem-local' ),
				'validate'     =>'not_empty',
			),
			array(
				...
				'type'         => 'text',
				'title'        => esc_html__( 'Электронная почта компаний', 'kem-local' ),
				'desc'         => esc_html__( 'Измените электронную почту компаний используя это поле', 'kem-local' ),
				'subtitle'     => esc_html__( 'Эл. почта', 'kem-local' ),
				'placeholder'  => esc_html__( 'Поле для ввода ', 'kem-local' ),
				'validate'	   => array( 'not_empty','email' ),
				'hint' => array(
						'title'   => 'Подсказка',
						'content' => 'Введите существующую электронную почту',
				),
			),
			array(
				'id'           => 'footer-contacts-phone',
				'type'         => 'text',
				'title'        => esc_html__( 'Контактный телефон компаний', 'kem-local' ),
				'desc'         => esc_html__( 'Измените контактный телефон компаний используя это поле', 'kem-local' ),
				'subtitle'     => esc_html__( 'Телефон', 'kem-local' ),
				'placeholder'  => esc_html__( 'Поле для ввода ', 'kem-local' ),
				'validate'	   => array( 'not_empty','numeric' ),
				'hint' => array(
						'title'   => 'Подсказка',
						'content' => 'Поле должно состоять из цифр и знака +, без пробелов',
				),
			),
		),
	)
...
// 	$opt_name,
// 	array(
// 		'title'            => esc_html__( 'Editors', 'kem-local' ),
// 		'id'               => 'editor',
...
// 	$opt_name,
// 	array(
// 		'title' => esc_html__( 'Color Selection', 'kem-local' ),
// 		'id'    => 'color',
// 		'icon'  => 'el el-brush',
// 	)
// );
...
// 	$opt_name,
// 	array(
// 		'title' => esc_html__( 'Design Fields', 'kem-local' ),
// 		'id'    => 'design',
// 		'icon'  => 'el el-wrench',
// 	)
// );
...
// 	$opt_name,
// 	array(
// 		'title' => esc_html__( 'Media Uploads', 'kem-local' ),
// 		'id'    => 'media',
// 		'icon'  => 'el el-picture',
// 	)
// );
...
// 	$opt_name,
// 	array(
// 		'title' => esc_html__( 'Presentation Fields', 'kem-local' ),
// 		'id'    => 'presentation',
// 		'icon'  => 'el el-screen',
// 	)
// );
...
// 	$opt_name,
// 	array(
// 		'title' => esc_html__( 'Switch / Button Set', 'kem-local' ),
// 		'id'    => 'switch_buttonset',
// 		'icon'  => 'el el-cogs',
// 	)
// );
...
// 	$opt_name,
// 	array(
// 		'title' => esc_html__( 'Select Fields', 'kem-local' ),
// 		'id'    => 'select',
// 		'icon'  => 'el el-list-alt',
// 	)
// );
...
// 	$opt_name,
// 	array(
// 		'title' => esc_html__( 'Slider / Spinner', 'kem-local' ),
// 		'id'    => 'slider_spinner',
...
// 	$opt_name,
// 	array(
// 		'title' => esc_html__( 'Additional Types', 'kem-local' ),
// 		'id'    => 'additional',
// 		'icon'  => 'el el-magic',
// 	)
// );
...
// 	$opt_name,
// 	array(
// 		'title' => esc_html__( 'Advanced Features', 'kem-local' ),
...
// 	$opt_name,
// 	array(
// 		'title' => esc_html__( 'Disabling', 'kem-local' ),
// 		'icon'  => 'el el-lock',
// 	)
// );
...
if ( class_exists( 'Redux_Pro' ) ) {
	Redux::set_section(
		$opt_name,
		array(
			'title' => esc_html__( 'Redux Pro Fields', 'kem-local' ),
			'id'    => 'redux-pro-fields',
			'icon'  => 'el el-redux',
			'class' => 'pro_highlight',
			'desc'  => esc_html__( 'For full documentation on this field, visit: ', 'kem-local' ) . '<a href="//docs.redux.io/premium/" target="_blank">docs.redux.io/premium/</a>',
		)
	);
...
if ( file_exists( $dir . '/../README.md' ) ) {
	$section = array(
		'icon'   => 'el el-list-alt',
		'title'  => esc_html__( 'Documentation', 'kem-local' ),
		'fields' => array(
			array(
...
Redux::set_section(
	...
	array(
		'icon'            => 'el el-list-alt',
		'title'           => esc_html__( 'Customizer Only', 'kem-local' ),
		'desc'            => '<p class="description">' . esc_html__( 'This Section should be visible only in Customizer', 'kem-local' ) . '</p>',
		'customizer_only' => true,
		'fields'          => array(
			array(
				'id'              => 'opt-customizer-only',
				'type'            => 'select',
				'title'           => esc_html__( 'Customizer Only Option', 'kem-local' ),
				'subtitle'        => esc_html__( 'The subtitle is NOT visible in customizer', 'kem-local' ),
				'desc'            => esc_html__( 'The field desc is NOT visible in customizer.', 'kem-local' ),
				'customizer_only' => true,
				'options'         => array(
					'1' => esc_html__( 'Opt 1', 'kem-local' ),
					'2' => esc_html__( 'Opt 2', 'kem-local' ),
					'3' => esc_html__( 'Opt 3', 'kem-local' ),
				),
				'default'         => '2',
			),
		),
...
if ( ! function_exists( 'dynamic_section' ) ) {
	...
	function dynamic_section( $sections ) {
		$sections[] = array(
			'title'  => esc_html__( 'Section via hook', 'kem-local' ),
			'desc'   => '<p class="description">' . esc_html__( 'This is a section created by adding a filter to the sections array. Can be used by child themes to add/remove sections from the options.', 'kem-local' ) . '</p>',
...
if ( ! function_exists( 'change_defaults' ) ) {
	...
	function change_defaults( $defaults ) {
		$defaults['str_replace'] = esc_html__( 'Testing filter hook!', 'kem-local' );
